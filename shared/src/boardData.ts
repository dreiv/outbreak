...
// 2:1 equirectangular canvas; city x/y derived from real lat/lon so the
// landmass layer (client/src/worldLand.ts, same projection) lines up exactly.
...
// `major: true` = label shown at low zoom; others reveal on zoom-in.
const RAW_CITIES: RawCity[] = [
  ...
  { id: "milan", name: "Milan", region: "azure", lat: 45.4642, lon: 9.19 },
  ...
  { id: "seoul", name: "Seoul", region: "crimson", lat: 37.5665, lon: 126.978 },
  {
    id: "tokyo",
    name: "Tokyo",
    region: "crimson",
  ...
  { id: "bogota", name: "Bogotá", region: "amber", lat: 4.711, lon: -74.0721 },
...
// Long-haul wrap-around routes (like Pandemic's dashed LA<->Tokyo lines).
// Functionally normal edges, but rendered dashed as the "long way round".
...
// Undirected edges; both directions derived automatically below.
...
export const CITY_MAP: Record<string, CityDef> = Object.fromEntries(
  CITIES.map((c) => [c.id, c]),
);
