...
import { CITIES, CITY_MAP, STARTING_CITY, isConnected } from '../../shared/src/boardData';
import {
  GameState, Player, PlayerAction, PlayerCard, RegionId, RoleId,
  ROLES, HAND_LIMIT,
...
const DEAL_SIZE: Record<number, number> = { 2: 4, 3: 3, 4: 3, 5: 2, 6: 2, 7: 2 };
...
export function createRoom(roomId: string): Room {
  const state: GameState = {
    ...
    diseaseState: { azure: 'active', crimson: 'active', amber: 'active', verdant: 'active' },
...
export function addPlayer(room: Room, playerId: string, name: string): string | null {
  ...
  room.state.players.push({
    id: playerId, name, role: null, location: STARTING_CITY, hand: [], connected: true,
  });
...
export function startGame(room: Room): string | null {
  ...
  // Assign roles
  const roleIds = shuffle(ROLES.map((r) => r.id)).slice(0, state.players.length) as RoleId[];
  state.players.forEach((p, i) => { p.role = roleIds[i]; p.location = STARTING_CITY; p.hand = []; });

  // Build & shuffle infection deck
  const infectionDeck = shuffle(CITIES.map((c) => c.id));

  // Initial infection: 3/3/3 cities get 3/2/1 cubes
  const internal: RoomInternal = { playerDeck: [], infectionDeck, freeMoveUsed: new Set() };
  for (const amount of [3, 2, 1]) {
    for (let i = 0; i < 3; i++) {
      ...
      for (let k = 0; k < amount; k++) addCubeToCity(state, cityId, region, new Set());
    }
  }

  // Build player deck: one card per city, shuffle, deal hands, then seed epidemics
  const cityCards: PlayerCard[] = shuffle(CITIES.map((c) => ({ type: 'city', city: c.id, uid: randomUUID() } as const)));
  ...
  const piles: PlayerCard[][] = Array.from({ length: EPIDEMIC_COUNT }, () => []);
  cityCards.forEach((card, i) => piles[i % EPIDEMIC_COUNT].push(card));
  const deckWithEpidemics: PlayerCard[] = [];
  for (const pile of piles) {
    const withEpidemic = shuffle([...pile, { type: 'epidemic', uid: randomUUID() } as const]);
...
function addCubeToCity(state: GameState, city: string, region: RegionId, visited: Set<string>) {
  ...
  if (cur >= 3) {
    ...
    log(state, `Outbreak in ${CITY_MAP[city].name}! (${state.outbreakCounter}/${state.outbreakMax})`);
    if (state.outbreakCounter >= state.outbreakMax) {
      state.phase = 'lost';
      state.lossReason = 'The outbreak counter reached its maximum — containment has failed.';
      return;
    }
...
function totalCubesOfRegion(state: GameState, region: RegionId): number {
  let sum = 0;
  for (const city of Object.keys(state.cityCubes)) sum += state.cityCubes[city]?.[region] ?? 0;
  return sum;
}
...
function resolveEpidemic(room: Room) {
  ...
  // 1. Increase
  state.infectionRateIndex = Math.min(state.infectionRateIndex + 1, state.infectionRateTrack.length - 1);
...
function findCardIndex(hand: PlayerCard[], predicate: (c: PlayerCard) => boolean): number {
  return hand.findIndex(predicate);
}

export function applyAction(room: Room, playerId: string, action: PlayerAction): string | null {
  const { state, internal } = room;
  ...
  if (state.pendingDiscard) return 'A discard is pending before the game can continue.';
  ...
  switch (action.type) {
    ...
    case 'drive': {
      ...
      if (!isConnected(player.location, action.to)) return 'That city is not connected.';
      player.location = action.to;
      const free = player.role === 'logistics-chief' && !internal.freeMoveUsed.has(player.id);
      if (free) internal.freeMoveUsed.add(player.id); else state.actionsRemaining--;
      log(state, `${player.name} traveled to ${CITY_MAP[action.to].name}.`);
      break;
    }
    case 'direct-flight': {
      if (state.actionsRemaining <= 0) return 'No actions remaining.';
      const idx = findCardIndex(player.hand, (c) => c.type === 'city' && c.city === action.to);
    ...
    case 'charter-flight': {
      if (state.actionsRemaining <= 0) return 'No actions remaining.';
      const idx = findCardIndex(player.hand, (c) => c.type === 'city' && c.city === player.location);
      if (idx === -1) return 'You do not hold the card for your current city.';
      if (!CITY_MAP[action.to]) return 'Unknown destination.';
      player.hand.splice(idx, 1);
      player.location = action.to;
      state.actionsRemaining--;
      log(state, `${player.name} chartered a flight to ${CITY_MAP[action.to].name}.`);
      break;
    }
    case 'shuttle-flight': {
      if (state.actionsRemaining <= 0) return 'No actions remaining.';
      if (!state.researchStations.includes(player.location)) return 'No research station in your current city.';
      if (!state.researchStations.includes(action.to)) return 'No research station at the destination.';
      player.location = action.to;
      state.actionsRemaining--;
      log(state, `${player.name} took a shuttle flight to ${CITY_MAP[action.to].name}.`);
      break;
    }
    case 'treat': {
      if (state.actionsRemaining <= 0) return 'No actions remaining.';
      ...
      const removeAll = player.role === 'field-medic' || state.diseaseState[action.region] === 'cured';
      ...
      if (state.diseaseState[action.region] === 'cured' && totalCubesOfRegion(state, action.region) === 0) {
        state.diseaseState[action.region] = 'eradicated';
        log(state, `${action.region} strain eradicated!`);
      }
      state.actionsRemaining--;
      log(state, `${player.name} treated ${action.region} in ${CITY_MAP[player.location].name}.`);
      break;
    }
    case 'build-station': {
      if (state.actionsRemaining <= 0) return 'No actions remaining.';
      if (state.researchStations.includes(player.location)) return 'A station already exists here.';
      ...
      state.actionsRemaining--;
      log(state, `${player.name} built a research station in ${CITY_MAP[player.location].name}.`);
      break;
    }
    case 'share-knowledge': {
      ...
      const courierInvolved = player.role === 'courier' || other.role === 'courier';
      if (!colocated && !courierInvolved) return 'Both players must be in the same city.';
      const flexibleCard = player.role === 'liaison-officer' || other.role === 'liaison-officer';

      const giver = action.direction === 'give' ? player : other;
      const receiver = action.direction === 'give' ? other : player;
      const idx = findCardIndex(giver.hand, (c) => c.type === 'city' && c.city === action.cityCard);
      ...
      state.actionsRemaining--;
      log(state, `${player.name} ${action.direction === 'give' ? 'gave a card to' : 'took a card from'} ${other.name}.`);
      break;
    }
    case 'discover-cure': {
      if (state.actionsRemaining <= 0) return 'No actions remaining.';
      if (!state.researchStations.includes(player.location)) return 'You must be at a research station.';
      if (state.diseaseState[action.region] !== 'active') return 'That strain is already cured.';
      const needed = player.role === 'virologist' ? 4 : 5;
      if (action.cardUids.length !== needed) return `You must discard exactly ${needed} matching city cards.`;
      const cards: PlayerCard[] = [];
      for (const uid of action.cardUids) {
        const c = player.hand.find((h) => h.uid === uid);
        if (!c || c.type !== 'city' || CITY_MAP[c.city].region !== action.region) {
      ...
      state.actionsRemaining--;
      log(state, `${player.name} discovered the cure for the ${action.region} strain!`);
      checkWin(state);
      break;
    }
    default:
  ...
  if (state.phase === 'playing' && state.actionsRemaining <= 0 && !state.pendingDiscard) {
    endOfActions(room);
  }
  return null;
}
